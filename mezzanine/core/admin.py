...
# TODO 考虑稍后抽出去
def hotkeywords_for(*args):
    """
    the not decorated version of keywords_for
    """
...
class HotKeywordFilter(SimpleListFilter):
    title = '热门关键字'; #TODO use i18n
    parameter_name = 'hotkw';
    
    def lookups(self, request, model_admin):
        model = model_admin.model
        kws = hotkeywords_for("{0}.{1}".format(model._meta.app_label, model._meta.object_name).lower())
        return [(kw.id, "{}({})".format(kw.title, kw.item_count)) for kw in kws]

    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(keywords__keyword__exact=self.value())
        else:
            return queryset.all()
...
class DisplayableAdmin(BaseTranslationModelAdmin):
    ...
    #XXX list_filter = ("status", "keywords__keyword")
    list_filter = ("status", HotKeywordFilter)
